...
def SROArtifact(samples, XO_val, clockeffects_dict, samp_rate):
    ...
    # analog bandwidth
    f_ABW = clockeffects_dict['XOFreq'] * \
    ...
    # np.arange(0, samples_length * Ts, Ts)
    Ts_range = np.linspace(0, samples_length * Ts, samples_length)
    Ts_prime_range = Ts_range + \
        ((Ts*clockeffects_dict['TimetickScalingFactor']) /
         f_ABW)*SRO_val  # np.cumsum(Ts_prime)

...
def train_model(model, model_file, train_set, val_set, TrainParams):
    ...
    if optimizer_type == 'Adam':
        optimizer = optim.Adam(
            model.parameters(), lr=learning_rate, weight_decay=TrainParams['weight_decay'])
    ...
    scheduler = StepLR(
        optimizer, step_size=TrainParams['LRScheduler_stepsize'], gamma=TrainParams['LRSchedulerGamma'])
    ...
    if os.path.isfile(model_file):
        ...
        # Since we are already done with n_previous_epochs, we are starting with the next one.
        n_prev_epochs = n_prev_epochs + 1
        model.eval()
    ...
    for epoch in range(n_prev_epochs, num_epochs+1):
        ...
        with torch.no_grad():
            train_loss = calculate_loss(
                model, train_data, train_labels, batch_size_for_loss_calculation, computing_device, criterion)
            ...
            train_acc = calculate_accuracy(
                model, train_data, train_labels, batch_size_for_loss_calculation, computing_device)
            val_acc = calculate_accuracy(
                model, val_data, val_labels, batch_size_for_loss_calculation, computing_device)
        ...
        if (prev_val > val_loss):
            print("Previous best and current validation losses are: ",
                  prev_val, val_loss)
            torch.save(model.state_dict(), model_file)
            prev_val = val_loss
            stop_con = 0
        elif epoch > 1:
            print("Validation loss increased from %f to %f." %
                  (prev_val, val_loss))
            model.load_state_dict(torch.load(model_file))
            stop_con += 1
